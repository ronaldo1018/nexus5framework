    ...
    public final class WakeLock {
        ...
        public void acquire() {
            synchronized (mToken) {
                acquireLocked();
				try {
                    if(!mService.isScreenOn())	mHandler.postDelayed(mReleaser, 20*1000);
                } catch (RemoteException e) {
                }
            }
        }
        ...
        public void acquire(long timeout) {
            synchronized (mToken) {
				...
				/*try {
                    if(!mService.isScreenOn())	mHandler.postDelayed(mReleaser, min(1*1000, timeout));
					else mHandler.postDelayed(mReleaser, timeout);
                } catch (RemoteException e) {
                }*/
            }
        }
